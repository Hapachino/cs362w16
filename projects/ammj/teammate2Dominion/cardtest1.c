 ...
 * 	after playSmithy() function run in dominion.c
 */
 ...
 * 	to run the tests on playSmithy() function in dominion.c
 */
void setupTest(struct gameState *activeGame, struct cardResults *result){
	...
	// hand size is at least 1 as playSmithy assumes the card
 ...
 * 	running playSmithy() in dominion.c
 */
void cardtest1(int printVal, int seed, struct cardResults *result){
	...
	r = smithyCard(activeGame, result->curPlayer, result->advPos);
	assert(r == 0);
	result->testsPassed++;
