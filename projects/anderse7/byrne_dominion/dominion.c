...
int playAdventurer(int drawntreasure, struct gameState *state,int currentPlayer,int z,int temphand[MAX_HAND]){
  int cardDrawn;
...
int playVillage(int handPos,struct gameState *state,int currentPlayer){
  drawCard(currentPlayer, state);
...
int playGreat_hall(int handPos, struct gameState *state,int currentPlayer){
   //+1 Card
  drawCard(currentPlayer, state);
      //+1 Actions
  state->numActions--;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
    {
    case adventurer:
      playAdventurer(drawntreasure, state,currentPlayer,z,temphand);
      return 0;
    ...
    case smithy:
      if (playSmithy(currentPlayer, state, handPos))
        return 0;
      return 0;

    case village:
      playVillage(handPos, state, currentPlayer);
      return 0;
    ...
    case great_hall:
      playGreat_hall(handPos, state,currentPlayer);
      return 0;
