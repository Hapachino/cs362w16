 ...
 * Add them to your hand, then discard the other revealed cards.
 This version of lest loops many time and used random inputs for cards, 
 players, coins etc.
*/
...
int main (int argc, char** argv) {
  ...
  for (n = 0; n < 10; n++) { //loop 20000 times
  ...
  printf("Coin cards Found: ");
   ...
   // ----------- TEST 4: +2 coins --------------
    ...
    printf("Test 6: Make sure other players are uneffected\n");
      ...
      assert (testG.discardCount[testG.whoseTurn] == G.discardCount[testG.whoseTurn]);
      assert (testG.handCount[testG.whoseTurn] == G.handCount[testG.whoseTurn]);
      assert (testG.deckCount[currPlayer] == G.deckCount[currPlayer]);
    #endif
