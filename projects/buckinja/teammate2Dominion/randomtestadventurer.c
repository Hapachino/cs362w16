...
void testAdventurer(struct gameState *G, int cardsInPlay[10]) {
    ...
    if(numberInHand == G->handCount[player] - 2)
		...
		printf("testAdventurer(): PASS hand size did not increment correctly. Number in hand before: %d, after: %d\n", numberInHand, G->handCount[player]);
		printf("Handcount should have incremented by 2.\n");
		printf("Number in deck: %d\n", G->deckCount[player]);
		printf("Previous number in deck: %d\n", numberInDeck);
		printf("Number treasure before: %d\n", numTreasure1);
		printf("Number treasure after: %d\n", numTreasureAfter1);
		exit(0);
	}
