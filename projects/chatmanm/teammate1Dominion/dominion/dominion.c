...
//Append CS362W16 Todd Waddell waddelto
/*******************************
 * Refactored:
...
int adventureEffect (struct gameState *state)
{
    ...
    int drawntreasure = 0;
    int cardDrawn =  0;
    while(drawntreasure < 2){ //bug 1-2: updated inequality to not equal 2.
...
{  
      ...
      for (int i = 0; i < 3; i++)
	{
      ...
      //discard card from hand
      discardCard(handPos, currentPlayer, state, 1); //bug3: changed trashFlag value to 0 so that card is not trashed and registers as a played card.
      return 0;
}		
