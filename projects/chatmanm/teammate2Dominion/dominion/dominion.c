...
int playAdventurer(int currentPlayer, struct gameState *state){
     ...
     int drawntreasure = 0; //bug 1: changed the initialize value from 1 to 0
     int cardDrawn;
...
int playSmithy(int currentPlayer, struct gameState *state, int handPos){
     ...
     int i;
     //+3 Cards
     for (i = 0; i < 3; i++) //bug 2: changed i = 1 to i = 0
     {
          drawCard(currentPlayer, state);
     }
...
int playVillage(int currentPlayer, struct gameState *state, int handPos){
     ...
     //+2 Actions
     state->numActions = state->numActions + 2; // bug 3: changed numActions + 3 to numAction + 2
