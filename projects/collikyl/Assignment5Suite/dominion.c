...
void play_smithy(int currentPlayer,struct gameState *state,int handPos)
{
      ...
      discardCard(handPos, currentPlayer, state, 0); //Changed to discard first. (violates rule) FIXD
     //+3 Cards
	 int i;
		for (i = 0; i < 3; i++)
		{
...
void play_adventurer(int currentPlayer,struct gameState *state,int handPos)
{
    ...
    while(z-1>=0)
	{ //Changed to not discard proper amount of cards. FIXD
...
void play_village(int currentPlayer,struct gameState *state,int handPos)
{
		...
		discardCard(handPos, currentPlayer, state, 0); //attempts to discard wrong card. FIXD
}
...
void play_salvager(int currentPlayer,struct gameState *state,int handPos, int choice1)
{
      ...
      state->numBuys++;
		...
		//trash card removed for error.... FIXD
			discardCard(handPos, currentPlayer, state, 1);
		}
			
		//discard card
		discardCard(handPos, currentPlayer, state, 0); //FIXD
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case sea_hag:
	play_seaHag(currentPlayer,state,handPos);
      return 0;
