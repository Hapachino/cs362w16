...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card )
	...
	printf("\n****** IN ADV CASE*****\n");
	printHand(0,state);
	printf("***************END OF ADV CASE\n");

    case council_room:
      //+4 Cards
      printf("IN COUNCIL ROOM\n");
      for (i = 0; i < 4; i++)
	{
...
int adventurerCard(struct gameState *state, int handPos)
{
  ...
  int z = 0;// this is the counter for the temp hand
	...
	printf("******************END OF ADV FUNCTION\n");
      return 0;
}

smithyCard(struct gameState *state, int handPos)
{
  int i;
  int currentPlayer = whoseTurn(state);
...
villageCard(struct gameState *state, int handPos)
{
  int currentPlayer = whoseTurn(state);
...
greatHallCard(struct gameState *state, int handPos)
{
  int currentPlayer = whoseTurn(state);
...
outpostCard(struct gameState *state, int handPos)
{
  int currentPlayer = whoseTurn(state);
