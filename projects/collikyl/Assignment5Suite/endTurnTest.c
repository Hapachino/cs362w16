...
void testPlayed(struct gameState *preTest,struct gameState *postTest)
{
	...
	else if(diff != TESTCARDVAL)
	{
		printf("PlayedCard val incorrect. Played wrong card!\n");
		flag = 1;
	}
	for(i = 0; i <= preTest->playedCardCount -1; i++) //WILL ONLY DISPLAY IF TOO MANY CARDS IN PLAYED CARD PILE, AND THERE IS A WRONG CARD IN THE PILE
	{
