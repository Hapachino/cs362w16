...
{
	int handSize = rand() % 5; //handSize 0-49 last card in hand will be tested card
	int i,card;
...
int coinsCount(struct gameState *pre, struct gameState *post)
{
	...
	for(i=pre->handCount[0]-1;i<post->handCount[0]; i++)
	{
...
void testHand(struct gameState *pre, struct gameState *post, int player, int isShuffle, int compareTreasure)
{
	...
	for(i=0;i<=1;i++)
	{
		...
		for(j=0;j<preHandCount-1;j++) //Check Previous cards in hand to see if they changed
		{
...
//		printf("\nPre Deck Count %d\n",preTest.deckCount[player]);
		printDeck(player,&preTest);
		playCard((postTest.handCount[player]-1),0,0,0,&postTest);
//		printf("\nPost Hand Count %d\n",postTest.handCount[player]);
		printHand(player,&postTest);
//		printf("\nPost Deck Count %d\n",postTest.deckCount[player]);
		printDeck(player,&postTest);
