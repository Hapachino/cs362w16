...
void play_smithy(currentPlayer,state,handPos)
{
      ...
      discardCard(handPos, currentPlayer, state, 0); //Changed to discard first. (violates rule)
     //+3 Cards
	 int i;
		for (i = 0; i < 3; i++)
		{
...
void play_adventurer(currentPlayer,state,handPos)
{
    ...
    while(z-1>0)
	{ //Changed to not discard proper amount of cards.
...
void play_village(currentPlayer,state,handPos)
{
		...
		discardCard(handPos-1, currentPlayer, state, 0); //attempts to discard wrong card.
}
