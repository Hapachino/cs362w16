Smithy Card (incorrect):
    Made its own function: cSmithy(int currentPlayer, struct gameState *state, int handPos)
    Changed number of cards drawn to 2 instead of 3.

Adventurer Card (incorrect):
    Made its own function: cAdventurer(int drawntreasure, struct gameState *state, int currentPlayer, int cardDrawn, int * temphand, int z)
    Changed if statement to remove copper.

Village Card (incorrect):
    Made its own function: cVillage(int currentPlayer, struct gameState *state, int handPos)
    Change flag from discardCard() from 0 to 1.

Feast Card (incorrect):
    Made its own function: cFeast(struct gameState *state, int choice1, int currentPlayer, int handPos)
...
Council Room Card (correct):
    Made its own function: cCouncil_Room(struct gameState *state, int currentPlayer, int cardDrawn, int z, int handPos)
    No other changes.
