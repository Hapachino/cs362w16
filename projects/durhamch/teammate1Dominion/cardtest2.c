 ...
 * 	after adventurerCard() function run in dominion.c
 */
 ...
 * 	to run the tests on adventurerCard() function in dominion.c
 */
void setupTest(struct gameState *activeGame, struct cardResults *result){
	...
	// hand size is at least 1 as adventurerCard assumes the card
 ...
 * 	running adventurerCard() in dominion.c
 */
void cardTest2(int printVal, int seed, struct cardResults *result){
	...
	r = adventurerCard(drawntreasure, activeGame, result->curPlayer, cardDrawn, temphand, z);
	assert(r == 0);
	result->testsPassed++;
