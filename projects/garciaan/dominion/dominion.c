...
int endTurn(struct gameState *state) {
  ...
  for (i = 0; i < state->playedCardCount; i++){
    discard_count = state->discardCount[currentPlayer];
    state->discard[currentPlayer][discard_count] = state->playedCards[i];
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      if (play_adventurer(state, currentPlayer, handPos) == -1){
        return -1;
      }
      return 0;
    ...
    case village:
      play_village(state, currentPlayer, handPos);
      return 0;
    ...
    case great_hall:
      play_great_hall(state, currentPlayer, handPos);
      return 0;
