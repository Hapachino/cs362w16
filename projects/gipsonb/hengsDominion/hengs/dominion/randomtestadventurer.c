/* -----------------------------------------------------------------------
 * Random test for Adventurer Card. Pretty much identical to Village except
 * with on less action.
 * -----------------------------------------------------------------------
 */
...
int main() {
  ...
  for (n = 0; n < 5000; n++) { //runs 5000 tests
	  ...
	  p = floor(Random() * 2);
	  game.deckCount[p] = floor(Random() * MAX_DECK); //randomly sets deckcount
	  game.discardCount[p] = floor(Random() * MAX_DECK); //randomly sets discardCount
	  game.handCount[p] = floor(Random() * MAX_HAND); //randomly sets handCount
	  for (j = 0; j < 3; j++) {
...
int testAdventurer(struct gameState *post, int p){
	int carddrawn, r, i, z = 0, drawntreasure = 0;
	int temphand[post->handCount[p]];
	struct gameState pre;
	
	memcpy (&pre, post, sizeof(struct gameState));
	
	
	r = playAdventurer(post, p, drawntreasure, carddrawn, temphand, z);
