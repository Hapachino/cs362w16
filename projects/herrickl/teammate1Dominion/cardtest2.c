...
int main(){
	...
	if(Test.handCount[p1] == Clean.handCount[p1]+2){ //acount for discard of this card
		printf("Passed. Hand count is %d + 2 = %d\n", Clean.handCount[p1], Test.handCount[p1]);
	} 
	else{  //fail
		printf("Failed. Hand count is %d + 2= %d\n", Clean.handCount[p1], Test.handCount[p1]);
	}
	testNum++;
	...
	if(Test.deckCount[p1] == Clean.deckCount[p1] - (Test.discardCount[p1]+2)){	//discards plus two treasure cards
		printf("Passed. Deck count is %d + 2 = %d\n", Clean.deckCount[p1], Test.deckCount[p1]);
	}
	else{	//fail
		printf("Failed. Deck count is %d + 2 = %d\n", Clean.deckCount[p1], Test.deckCount[p1]);
	}
	testNum++;
