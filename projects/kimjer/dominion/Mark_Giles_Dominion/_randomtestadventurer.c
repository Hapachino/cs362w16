...
//adventurerEffect(int currentPlayer, int drawntreasure, int cardDrawn, int temphand[], struct gameState *state) {

		handCountExp = handCount + 2;
		ret = adventurerEffect(infos.currentPlayer, infos.drawntreasure, infos.cardDrawn, infos.temphand, g);
		handCount = g->handCount[infos.currentPlayer];
		deckCount = g->deckCount[infos.currentPlayer];
		discardCount = g->discardCount[infos.currentPlayer];
