...
{	
  puts("heyo");
      if (1){//Debug statements
      printf("--Current deck count before: %d\n", state->deckCount[player]);
    }
    if (1){//Debug statements
      printf("--Current hand count before: %d\n", state->handCount[player]);
    }
  int count = 0;
  int deckCounter = 0;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  struct infosStruct *infos = malloc(sizeof(struct infosStruct));
  int i;
  int j;
  int k;
  int x;
  ...
  if (nextPlayer > (state->numPlayers - 1)){
    nextPlayer = 0;
    infos->nextPlayer = nextPlayer;
  }
  
  infos->z = 0;
  infos->currentPlayer = whoseTurn(state);
  infos->tributeRevealedCards[0] = -1;
  infos->tributeRevealedCards[1] = -1;
  infos->drawntreasure = 0;
  infos->handPos = handPos;
  infos->choice1 = choice1;
  infos->choice2 = choice2;
  infos->choice3 = choice3;
  //uses switch to select card and perform actions
  switch( card ) 
    {
    case adventurer:
      return effectAdventure(state, infos);

    case council_room:
      return effectCouncil(state, infos);
    ...
    case remodel:
      return effectRemodel(state, infos);
		
    case smithy:
      return effectSmithy(state, infos);

		case village:
      return effectVillage(state, infos);
...
{
  int currentPlayer = infos->currentPlayer;
  int choice1 = infos->choice1;
  int choice2 = infos->choice2;
