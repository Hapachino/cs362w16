 ...
 *  Jennifer Mendoza
 *	CS 325
 *	Description: This is a test program that tests the playFeast() function.
 *	
 ...
 * 
 * randomtestFeast: refactoredCardTest.c dominion.o rngs.o
 *      gcc -o testFeast -g  refactoredCardTest.c dominion.o rngs.o $(CFLAGS)
 *
 * -----------------------------------------------------------------------
 */
#include "dominion.h"
...
void testFeast(struct gameState *post,int p, int choice) {
	...
	feastCard(p,post,0);
#if (NOISY_TEST == 1)
	printf("Testing when player %d plays Feast card\n",p);
	if(p==1)
	{
	...
	if (p==1)
	{
		printf("Testing if player 1 handCount increased by 1");
		if (pre.handCount[1]+1==post->handCount[1])
		{
			printf("....PASSED\n");
	...
	if (p==2)
	{
		...
		if (pre.handCount[2]+1==post->handCount[2])
		{
			printf("....PASSED\n");
			printf("pre: %d, post:%d\n", pre.handCount[2],post->handCount[2]);
		}
 ...
 int main () {
  ...
  for (n = 0; n < 20; n++) {
	...
	choice= rand() % MAX_DECK;
    testFeast(&G,p,choice);
  }
