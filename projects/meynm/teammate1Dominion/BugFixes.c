...
------------------------------------------------------------------------------------
 ...
 Running the test, which I have built as testSmithy, I have reproduced the failures
 reported.
 ...
 Running testSmithy again confirms this bug has been resolved. The corrected 
 function is in folder meynm/dominion_fixed
 ...
 When fixes to Bugs #1 and #2 are applied, testSmithy runs with no failures.
 ...
 I have reproduced this failure using the test, which I have built as testAdv.
 ...
 Running testAdv again, handCount and coin total are correctly updated. The project,
 with corrected code, is in meynm/dominion_fixed
 ...
 Running unit test 4, I have reproduced the failures, which seem to result from 
 several bugs in the game code.
 ...
 other cards have been counted, I apply the appropriate bonus for the Gardens card,
 ...
 function looks like this:
	...
	Rerunning the tests confirms that the bugs are resolved. The correct ScoreFor()
	function is in meynm/dominion_fixed
 ...
 Bug #5
    ...
    Platform: Ubuntu Linux 15.04 running Dominion in Code::Blocks IDE
    Can it be reproduced?: Yes
    Description: After a player plays the Salvager card, the coin count is often 
    incorrect by varying amounts.
    
    Steps to reproduce: Go into the teammate1dominion folder and run command "make 
    cardtest3.out". Note that this test takes a few seconds to run.
                        
    Expected results: All coin counts should be accurately calculated because the 
    Salvager card should correctly give the user the value of their trashed card.
                     
    Actual results: The value of the trashed card seems to not be correctly 
    calculated.
  ...
  This fix causes the tests to allocate the correct number of coins, as is confirmed
  by cardtest3. The corrected cardEffect() function is along with the rest of the 
  fixed project, in meynm/dominion_fixed
