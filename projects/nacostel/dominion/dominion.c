...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  if (nextPlayer > (state->numPlayers - 1)){
   ...
   if(card == smithy)
	   smithyCard(currentPlayer, state, handPos, i);

   if(card == council_room)
	   councilRoomCard(currentPlayer, state, handPos, i);

   if(card == feast)
	   feastCard(state, currentPlayer, temphand, choice1, i, x);

   if(card == mine)
	   mineCard(state, currentPlayer, choice1, choice2, curse, handPos, j, i);
...
int smithyCard(int currentPlayer, struct gameState *state, int handPos, int i){
	//+3 Cards
	for (i = 0; i < 3; i++)
	{
		 drawCard(currentPlayer, state);
...
int feastCard(struct gameState *state, int currentPlayer, int temphand[], int choice1, int i, int x){
...
int mineCard(struct gameState *state, int currentPlayer, int choice1, int choice2, int curse, int handPos, int j, int i){
 j = state->hand[currentPlayer][choice1];  //store card we will trash
