...
void scoreForTest(int expectedScore, int handCards[10], int discardCards[10], int deckCards[10]){
    ...
    {
        int test = 9;
        printf("\nFAIL: Player's score was not calculated correctly with %d cards in hand, %d in discard, and %d in deck.", state.handCount[0], state.discardCount[0], state.deckCount[0]);
