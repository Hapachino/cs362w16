...
int main() {
	...
	for (j = 0; j < 10; j++){
		...
		playSmithy(&Game, Game.handCount[player]-1);
		...
		if (Game.handCount[player] != Test.handCount[player] + 2){
			printf("Error - Hand Count is not as expected. Test: %d, Actual: %d, Expected: %d\n", j, Game.handCount[player], Test.handCount[player] + 2);
		}
		...
		if (Test.deckCount[player] >= 3){
			if (Game.discard[player][Game.discardCount[player]] != smithy){
				printf("Error - We have a Smithy on the loose! (aka, it's not in Discard.)\n");
			}
		}
