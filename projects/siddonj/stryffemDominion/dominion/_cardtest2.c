...
void testPlayAdventurer() {
  ...
  adventurerCard(testGame->whoseTurn, testGame);         // First card is adventurer;
  ...
  memcpy(testGame, state, sizeof(struct gameState));            // Clean out previous state.
  int failPlay = adventurerCard(testGame->whoseTurn, testGame);   // Don't put adventurer card in player 1's hand.
  ...
  failPlay = adventurerCard(testGame->whoseTurn, testGame);          // Play adventurer with empty deck.
  ...
  failPlay = adventurerCard(testGame->whoseTurn, testGame);          // Play adventurer with 1 copper in deck.
