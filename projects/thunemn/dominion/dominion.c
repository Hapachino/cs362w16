...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      return playAdventurerCard(currentPlayer, state, drawntreasure, cardDrawn, temphand, z);
			
    case council_room:
      return playCouncilRoom(currentPlayer, state, numBuys, numPlayers, handPos);
    ...
    case great_hall:
      return playGreatHall(currentPlayer, state, numActions, handPos);
...
int playAdventurerCard(int currentPlayer, struct gameState *state, int drawntreasure, 
...
int playSmithCard(int currentPlayer, struct gameState *state, int handPos) {
  int i;
  for(i = 1; i < 3; ++i) {
    drawCard(currentPlayer, state);
  }

  //discard card ffrom hand
  discardCard(handPos, currentPlayer, state, 1);

  return 0;
}

int playCouncilRoom(int currentPlayer, gameState *state, int numBuys, int numPlayers, int handPos) {
  //+4 Cards
  int i;
  for (i = 0; i < 5; i++)
  {
...
int playGreatHall(int currentPlayer, gameState *state, int numActions, int handPos) {
  //+1 Card
  drawCard(currentPlayer, state);
