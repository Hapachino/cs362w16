...
int testAdventurer (int p, struct gameState *G) {
	...
	if (treasureCount == 0) {
		...
		printf("deck count" ) ;
		if (controlG.deckCount[p]  == G->deckCount[p])  
			printf("adventurer: FAIL on deckcount decreased.\n");
		else 
			printf("adventurer: PASS on deckcount decreased.\n");

		printf("discard count");
		if (controlG.discardCount[p] == G->discardCount[p] )  
			printf("adventurer: FAIL on discardcount changes.\n");
		else 
			printf("adventurer: PASS on discardcount changes.\n");
