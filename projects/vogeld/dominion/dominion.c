...
//Refactored cases from original cardEffect() switch statement: playAdventurer, playSmithy, playFeast, playRemodel, and playBaron
//Refactored case adventurer
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
  ...
  switch (card) {
    ...
    case great_hall: //+1 Card      
      drawCard(currentPlayer, state);      
      state->numActions++; //+1 Actions      
      discardCard(handPos, currentPlayer, state, 0); //discard card from hand
      return 0;

    case minion: //+1 action      
      state->numActions++;      
      discardCard(handPos, currentPlayer, state, 0); //discard card from hand
      if (choice1) //+2 coins
    ...
    case outpost:      
      ...
      discardCard(handPos, currentPlayer, state, 0); //discard card
      return 0;

    case salvager: //+1 buy      
      state->numBuys++;
      if (choice1) { //gain coins equal to trashed card        
        state->coins = state->coins + getCost(handCard(choice1, state));        
        discardCard(choice1, currentPlayer, state, 1); //trash card
      }      
      discardCard(handPos, currentPlayer, state, 0); //discard card
      return 0;
    ...
    case treasure_map:      
      index = -1; //search hand for another treasure_map
      for (i = 0; i < state->handCount[currentPlayer]; i++) {
...
int discardCard(int handPos, int currentPlayer, struct gameState *state, int trashFlag) {
  ...
  if (trashFlag < 1) { //add card to played pile    
...
int updateCoins(int player, struct gameState *state, int bonus) {
  int i;  
  state->coins = 0; //reset coin count
