 ...
 *   - Current player should gain exactly 1 actions.
...
int main() {
  ...
  int actualActionsGained = 0;    //how many actions great_hall actually gained
  int expectedActionsGained = 1;  //great_hall was supposed to gain 1 action
  int playedActions = 1;          //1 action card will have been played
  ...
  cardEffect(great_hall, choice1, choice2, choice3, &testGameState, handPos, &bonus);
  ...
  /* ---------------------- TEST 3 --------------------- */
  //current player should gain exactly 1 action.
  printf("-------------- TEST 3: TESTING THAT PLAYER RECEIVES 1 ACTION --------------\n");
  //player will have lost 1 action and gained 1
  actualActionsGained = testGameState.numActions - playedActions;
  //player starts with 1 action
  expectedActionsGained = actualGameState.numActions;
  ...
  if (totalNumberOfTests == successfulTests) {
    printf("************ SUCCESS: ALL TESTS PASSED FOR GREAT_HALL CARD ************\n");
  }
  else {
    printf("************ FAIL: NOT ALL TESTS PASSED FOR GREAT_HALL CARD ************\n");
  }
