...
// #define RAND_SEED 1000 //random seed for game init
...
number generator. */
void myShuffle(int *rooms, int n) {
  struct timeval t1;
...
int main() {
  ...
  int victory_Cards[3] = {ESTATE, DUCHY, PROVINCE}; //to be shuffled
  // int all_vict_cards[] = {CURSE, ESTATE, DUCHY, PROVINCE};
  // int all_tres_cards[] = {COPPER, SILVER, GOLD};

  //random test loop
  for (i = 0; i < 1000; i++) {
    myShuffle(all_k_cards, 19);
    //copy first 10 cards
    for (j = 0; j < 10; j++) {
    ...
    playSmithy(TEST_PLAYER, &testGameState, 0); //testing ian's smithy refactor
    ...
    for (j = 0; j < 3; j++) {
      if (testGameState.supplyCount[victory_Cards[j]] == actualGameState.supplyCount[victory_Cards[j]]) {
    ...
    for (j = 0; j < 10; j++) {
      if (testGameState.supplyCount[kingdom_Cards[j]] == actualGameState.supplyCount[kingdom_Cards[j]]) {
