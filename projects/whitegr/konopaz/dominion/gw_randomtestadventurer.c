...
int checkAdventurerCard(int p, struct gameState *post) {
  ...
  printHand(who,post);
   ...
   if (pre.deckCount[who] <1){//if the deck is empty we need to shuffle discard and add to deck
	printf("Deck Requires Shuffling\n" );
     if(pre.deckCount[who]  == post->deckCount[who]){
