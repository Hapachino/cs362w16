 ...
 * cardtest3.c - unit test for Village Card. 
 *
 * Basic Requirements of Village
 *	- Add one card to the current player's hand
 *	- Add two actions to the game state
 *	- Place card played (Village) into current player's discard deck
 *  - Place card played (Village) into current player's playedCards
...
#include <stdbool.h>
 ...
 #define TESTCARD_VALUE village

void cardtest3_helper(struct gameState* test, struct gameState* control, int currentPlayer, bool shuffledCards);
...
int main()
{
	...
	// ----------- TEST 1: +2 actions and +1 card from full player deck --------------
	printf("TEST 1: +2 actions and +1 card from full player deck\n");
	...
	cardtest3_helper(&test, &control, currentPlayer, false);

	// ----------- TEST 2: +2 actions and +1 card from full player deck --------------
	printf("TEST 2: +2 actions and +1 card from full player deck\n");
	//set currentPlayer's hand and deck
	control.hand[currentPlayer][0] = TESTCARD_VALUE;
	control.hand[currentPlayer][1] = copper;
	control.hand[currentPlayer][2] = duchy;
	...
	// compare results of test with expected results
	cardtest3_helper(&test, &control, currentPlayer, true);
