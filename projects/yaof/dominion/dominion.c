...
int fullDeckCount(int player, int card, struct gameState *state) {
  ...
  count = state->handCount[player] + count;
  count = state->deckCount[player] + count;
  count = state->discardCount[player] + count;
  return count;
}
...
int playAdventurer(struct gameState *state) 
{
  ...
  while(drawntreasure<2)
  {
    ...
    {
      // if the deck is empty we'll have to shuffle the deck
