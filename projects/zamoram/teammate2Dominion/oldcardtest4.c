...
int main()
{
    ...
    printf ("TESTING playCardGreatHall():\n");
    for(j=0; j < tests; j++)
    {
        ...
        hallLoc = (int) (rand() % numHand);
        tempHand[hallLoc] = great_hall;
        ...
        playCardGreatHall(&game, 0, hallLoc);


#if (NOISY_TEST == 1)
        printf("\nAFTER PLAYED Great Hall\n");
        printf("temp: %d [", numcards);
        for(i = 0; i < 20; i++)
            printf("%d, ", tempCards[i]);
        printf("\b\b]\n");
        ...
        if(game.numActions == numAction + 1)
            testActionsCount++;
#if (NOISY_TEST == 1)
        else
        ...
        //take discard of great hall into consideration
        drawCard(0, &test);
        discardCard(hallLoc, 0, &test, 0);  //discard the card
        ...
        {
            qsort ((void*)(game.hand[0]), game.handCount[0], sizeof(int), int_cmp);
            qsort ((void*)(test.hand[0]), test.handCount[0], sizeof(int), int_cmp);
            ...
            for(i = 0; i < numDiscard; i++)
                {
                    ...
                    if(game.hand[0][hallLoc] == tempDiscard[i])
                    {
                        currentTest = 1;
                    }
                }
