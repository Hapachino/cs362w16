/* Paul Zotz
 * CS 362 Winter 2016
 * Assignment 3 - card test 4 - great hall
...
int main() {
    ...
    testG.hand[currentPlayer][0] = great_hall;
    cardEffect(great_hall, 0, 0, 0, &testG, 0, 0); // play great hall card on current player
    // check for added card (accounting for removal of great hall)
    ...
    // ----------- TEST 3: current player receives +1 actions --------------
	printf("TEST 3: current player receives +1 actions\n");
	memcpy(&testG, &G, sizeof(struct gameState));
	testG.hand[currentPlayer][0] = great_hall;
    cardEffect(great_hall, 0, 0, 0, &testG, 0, 0); // play great hall card on current player
