...
int main() {
    ...
    playAdventurer(currentPlayer, &testG);
    for (i = 1; i <= testG.handCount[currentPlayer]; i++)
    {
        ...
        else if (testG.hand[currentPlayer][testG.handCount[currentPlayer]-i] != curse && passed == 1)
        {
            ...
            printf("expected: %d actual: %d\n", 2, testG.discardCount[currentPlayer]);
            passed = 0;
        }
    ...
    memcpy(&testG, &G, sizeof(struct gameState));
    // stack hand with treasure so we know we have some to draw
    ...
    memcpy(&testG, &G, sizeof(struct gameState));
	playAdventurer(currentPlayer, &testG); // play adventurer card on current player
